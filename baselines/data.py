...
class MELDataset(Dataset):
    def __init__(self, type, lookup, tokenizer=None, qid2name=None, qid2attr=None, qid2idx=None) -> None:
        ...
        if online_bert:
            ...
            if entity_text_type == "name" or entity_text_type == "attr":
                ...
                self.entity_text_raw = np.load(
                    os.path.join(preprocess_dir, "entity-brief-raw_%s.npy" % type), mmap_mode=entity_mmap  # type: ignore
                )
            else:
            ...
            self.mention_text_feature = np.load(
                os.path.join(preprocess_dir, "mention-text-feature_%s.npy" % type), mmap_mode=mention_mmap  # type: ignore
            )
            print("#", end="")
        ...
        if mention_final_layer_name == "multimodal":
            self.mention_image = np.load(
                os.path.join(preprocess_dir, "mention-image-feature_%s.npy" % type), mmap_mode=mention_mmap  # type: ignore
            )
            print("#", end="")
        if entity_final_layer_name == "multimodal":
            self.entity_image = np.load(
                os.path.join(preprocess_dir, "entity-image-feature_%s.npy" % type), mmap_mode=entity_mmap  # type: ignore
            ).reshape((-1, num_candidates_model, resnet_embed_dim))
            print("#", end="")
